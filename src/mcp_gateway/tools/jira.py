...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def jira_get_comments(
        ...
        logger.info("jira_get_comments 호출: %s", issue_key)
        client = ctx.request_context.lifespan_context["jira_client"]
        raw = await client.get_comments(issue_key)
        comments = [JiraComment.from_raw(c) for c in raw.get("comments", [])]
        logger.info("jira_get_comments 완료: %s → %d건", issue_key, len(comments))
    ...
    async def jira_update_comment(
        ...
        """JIRA 이슈 댓글을 수정한다."""
        logger.info("jira_update_comment 호출: %s / %s", issue_key, comment_id)
        client = ctx.request_context.lifespan_context["jira_client"]
        adf_body = {"body": text_to_adf(body)}
        raw = await client.update_comment(issue_key, comment_id, adf_body)
        comment = JiraComment.from_raw(raw)
        logger.info("jira_update_comment 완료: %s / %s", issue_key, comment_id)
        return json.dumps(comment, ensure_ascii=False)
    ...
    async def jira_delete_comment(
        ...
        """JIRA 이슈 댓글을 삭제한다."""
        logger.info("jira_delete_comment 호출: %s / %s", issue_key, comment_id)
        client = ctx.request_context.lifespan_context["jira_client"]
        await client.delete_comment(issue_key, comment_id)
        logger.info("jira_delete_comment 완료: %s / %s", issue_key, comment_id)
        return json.dumps(
    ...
    async def jira_create_issue(
        ...
        """JIRA 이슈를 생성한다."""
        logger.info("jira_create_issue 호출: project=%s, summary=%s", project_key, summary)
        client = ctx.request_context.lifespan_context["jira_client"]
        fields: dict = {
        ...
        logger.info("jira_create_issue 완료: %s", raw.get("key", ""))
        return json.dumps(
            {
                "key": raw.get("key", ""),
                "id": raw.get("id", ""),
    ...
    async def jira_update_issue(
        ...
        logger.info("jira_update_issue 호출: %s", issue_key)
        client = ctx.request_context.lifespan_context["jira_client"]
        fields: dict = {}
        if summary:
        ...
        logger.info("jira_update_issue 완료: %s (%s)", issue_key, ", ".join(fields.keys()))
        return json.dumps(
    ...
    async def jira_search_issues(
        ...
        logger.info("jira_search_issues 호출: jql=%s, max_results=%d", jql, max_results)
        client = ctx.request_context.lifespan_context["jira_client"]
        ...
        logger.info("jira_search_issues 완료: %d건 조회", len(issues))
    ...
    async def jira_get_transitions(
        ...
        logger.info("jira_get_transitions 호출: %s", issue_key)
        client = ctx.request_context.lifespan_context["jira_client"]
        raw = await client.get_transitions(issue_key)
        transitions = [JiraTransition.from_raw(t) for t in raw.get("transitions", [])]
        logger.info("jira_get_transitions 완료: %s → %d건", issue_key, len(transitions))
    ...
    async def jira_transition_issue(
        ctx: Context,
        issue_key: Annotated[str, Field(description="JIRA 이슈 키 (예: APTI-5266)")],
        transition_id: Annotated[str, Field(description="트랜지션 ID (jira_get_transitions로 조회)")],
    ) -> str:
        """JIRA 이슈의 상태를 변경한다."""
        logger.info("jira_transition_issue 호출: %s → transition_id=%s", issue_key, transition_id)
        client = ctx.request_context.lifespan_context["jira_client"]
        await client.transition_issue(issue_key, transition_id)
        logger.info("jira_transition_issue 완료: %s", issue_key)
        return json.dumps(
