...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def github_list_repos(
        ...
        logger.info("github_list_repos: search=%s", search)
    ...
    async def github_get_repo(
        ...
        logger.info("github_get_repo: %s", owner_repo)
    ...
    async def github_get_file_contents(
        ...
        logger.info("github_get_file_contents: %s@%s %s", owner_repo, ref, file_path)
    ...
    @mcp.tool()
    async def github_get_repository_tree(
        ctx: Context,
        owner_repo: Annotated[str, Field(description="저장소 경로 (예: owner/repo)")],
        ...
        logger.info("github_get_repository_tree: %s path=%s ref=%s", owner_repo, path, ref)
    ...
    async def github_list_commits(
        ...
        logger.info("github_list_commits: %s sha=%s", owner_repo, sha)
    ...
    async def github_list_branches(
        ...
        logger.info("github_list_branches: %s", owner_repo)
    ...
    async def github_create_branch(
        ...
        logger.info("github_create_branch: %s %s ← %s", owner_repo, branch, sha)
    ...
    @mcp.tool()
    async def github_get_branch_diffs(
        ctx: Context,
        owner_repo: Annotated[str, Field(description="저장소 경로 (예: owner/repo)")],
        ...
        logger.info("github_get_branch_diffs: %s %s…%s", owner_repo, base, head)
    ...
    async def github_list_pull_requests(
        ...
        """GitHub PR 목록을 조회한다."""
        logger.info("github_list_pull_requests: %s state=%s", owner_repo, state)
    ...
    async def github_get_pull_request(
        ...
        logger.info("github_get_pull_request: %s #%d", owner_repo, pull_number)
    ...
    async def github_create_pull_request(
        ...
        """새 GitHub PR을 생성한다."""
        logger.info("github_create_pull_request: %s %s → %s title=%s", owner_repo, head, base, title)
    ...
    @mcp.tool()
    async def github_list_pr_changed_files(
        ctx: Context,
        owner_repo: Annotated[str, Field(description="저장소 경로 (예: owner/repo)")],
        pull_number: Annotated[int, Field(description="PR 번호")],
        ...
        logger.info("github_list_pr_changed_files: %s #%d", owner_repo, pull_number)
    ...
    @mcp.tool()
    async def github_get_pr_file_diff(
        ctx: Context,
        owner_repo: Annotated[str, Field(description="저장소 경로 (예: owner/repo)")],
        pull_number: Annotated[int, Field(description="PR 번호")],
        ...
        logger.info("github_get_pr_file_diff: %s #%d files=%s", owner_repo, pull_number, file_paths)
    ...
    async def github_merge_pull_request(
        ...
        """PR을 머지한다."""
        logger.info("github_merge_pull_request: %s #%d method=%s", owner_repo, pull_number, merge_method)
    ...
    async def github_list_reviews(
        ...
        """PR의 리뷰 목록을 조회한다."""
        logger.info("github_list_reviews: %s #%d", owner_repo, pull_number)
    ...
    async def github_create_review(
        ...
        logger.info("github_create_review: %s #%d event=%s", owner_repo, pull_number, event)
    ...
    @mcp.tool()
    async def github_list_review_comments(
        ctx: Context,
        owner_repo: Annotated[str, Field(description="저장소 경로 (예: owner/repo)")],
        pull_number: Annotated[int, Field(description="PR 번호")],
        per_page: Annotated[int, Field(description="최대 결과 수", default=50)] = 50,
        ...
        logger.info("github_list_review_comments: %s #%d", owner_repo, pull_number)
    ...
    @mcp.tool()
    async def github_create_review_comment(
        ctx: Context,
        owner_repo: Annotated[str, Field(description="저장소 경로 (예: owner/repo)")],
        pull_number: Annotated[int, Field(description="PR 번호")],
        ...
        logger.info("github_create_review_comment: %s #%d %s:%d", owner_repo, pull_number, path, line)
    ...
    async def github_create_comment(
        ...
        logger.info("github_create_comment: %s #%d", owner_repo, issue_number)
    ...
    async def github_create_or_update_file(
        ...
        logger.info("github_create_or_update_file: %s %s", owner_repo, file_path)
        client = _client(ctx)
    ...
    async def github_get_comments(
        ...
        logger.info("github_get_comments: %s #%d", owner_repo, issue_number)
