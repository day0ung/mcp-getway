...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def confluence_get_page(
        ...
        logger.info("confluence_get_page 호출: %s (format=%s)", page_id, body_format)
        ...
        logger.info("confluence_get_page 완료: %s", page_id)
    ...
    async def confluence_search(
        ...
        logger.info("confluence_search 호출: cql=%s, limit=%d", cql, limit)
        client = ctx.request_context.lifespan_context["confluence_client"]
        ...
        logger.info("confluence_search 완료: %d건", len(hits))
    ...
    async def confluence_create_page(
        ...
        logger.info("confluence_create_page 호출: space=%s, title=%s", space_key, title)
        client = ctx.request_context.lifespan_context["confluence_client"]
        ...
        page = ConfluencePage.from_raw(raw)
        logger.info("confluence_create_page 완료: id=%s", page.get("id", ""))
        return json.dumps(page, ensure_ascii=False)
    ...
    async def confluence_update_page(
        ...
        """
        logger.info("confluence_update_page 호출: %s", page_id)
        client = ctx.request_context.lifespan_context["confluence_client"]
        ...
        page = ConfluencePage.from_raw(raw)
        logger.info(
            "confluence_update_page 완료: %s (v%d → v%d)",
            page_id,
            current_version,
            next_version,
        )
    ...
    @mcp.tool()
    async def confluence_get_database(
        ctx: Context,
        ...
        """
        logger.info("confluence_get_database 호출: %s", database_id)
        client = ctx.request_context.lifespan_context["confluence_client"]
        raw = await client.get_database(database_id)
        db = ConfluenceDatabase.from_raw(raw)
        logger.info("confluence_get_database 완료: %s", database_id)
        return json.dumps(db, ensure_ascii=False)

    @mcp.tool()
    async def confluence_list_databases(
        ctx: Context,
        space_key: Annotated[
            str,
            Field(
        ...
        logger.info("confluence_list_databases 호출: space_key=%s, limit=%d", space_key, limit)
        client = ctx.request_context.lifespan_context["confluence_client"]
        ...
        logger.info("confluence_list_databases 완료: %d건", len(dbs))
        return json.dumps(result, ensure_ascii=False)
    ...
    async def confluence_delete_page(
        ...
        logger.info("confluence_delete_page 호출: %s", page_id)
        client = ctx.request_context.lifespan_context["confluence_client"]
        await client.delete_page(page_id)
        logger.info("confluence_delete_page 완료: %s", page_id)
        return json.dumps(
