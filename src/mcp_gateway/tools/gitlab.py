...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def gitlab_list_projects(
        ...
        logger.info("gitlab_list_projects: search=%s membership=%s", search, membership)
    ...
    async def gitlab_get_file_contents(
        ...
        logger.info("gitlab_get_file_contents: %s@%s %s", project_id, ref, file_path)
    ...
    @mcp.tool()
    async def gitlab_get_repository_tree(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        ...
        logger.info("gitlab_get_repository_tree: %s path=%s ref=%s recursive=%s", project_id, path, ref, recursive)
    ...
    async def gitlab_list_commits(
        ...
        logger.info("gitlab_list_commits: %s ref=%s", project_id, ref_name)
    ...
    async def gitlab_list_branches(
        ...
        logger.info("gitlab_list_branches: %s search=%s", project_id, search)
    ...
    async def gitlab_create_branch(
        ...
        logger.info("gitlab_create_branch: %s %s ← %s", project_id, branch, ref)
    ...
    @mcp.tool()
    async def gitlab_get_branch_diffs(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        ...
        logger.info("gitlab_get_branch_diffs: %s %s…%s straight=%s", project_id, from_ref, to_ref, straight)
    ...
    async def gitlab_list_merge_requests(
        ...
        """프로젝트의 MR 목록을 조회한다."""
        logger.info("gitlab_list_merge_requests: %s state=%s", project_id, state)
    ...
    async def gitlab_get_merge_request(
        ...
        """MR 상세 정보를 조회한다."""
        logger.info("gitlab_get_merge_request: %s !%d", project_id, merge_request_iid)
    ...
    async def gitlab_create_merge_request(
        ...
        logger.info(
            "gitlab_create_merge_request: %s %s → %s title=%s",
            project_id,
            source_branch,
            target_branch,
            title,
    ...
    @mcp.tool()
    async def gitlab_get_merge_request_diffs(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        ...
        logger.info("gitlab_get_merge_request_diffs: %s !%d page=%d", project_id, merge_request_iid, page)
    ...
    @mcp.tool()
    async def gitlab_list_merge_request_changed_files(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        merge_request_iid: Annotated[int, Field(description="MR iid")],
        ...
        diff 본문이 필요하면 gitlab_get_merge_request_file_diff 로 파일 단위로 가져와라.
        """
        import re

        logger.info("gitlab_list_merge_request_changed_files: %s !%d", project_id, merge_request_iid)
        raw = await _client(ctx).get_merge_request_diffs(
    ...
    @mcp.tool()
    async def gitlab_get_merge_request_file_diff(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        merge_request_iid: Annotated[int, Field(description="MR iid")],
        ...
        logger.info(
            "gitlab_get_merge_request_file_diff: %s !%d files=%s",
            project_id,
            merge_request_iid,
            file_paths,
        )
    ...
    async def gitlab_approve_merge_request(
        ...
        """MR을 승인(Approve)한다."""
        logger.info("gitlab_approve_merge_request: %s !%d", project_id, merge_request_iid)
    ...
    async def gitlab_merge_merge_request(
        ...
        """MR을 머지한다."""
        logger.info("gitlab_merge_merge_request: %s !%d squash=%s", project_id, merge_request_iid, squash)
    ...
    async def gitlab_create_note(
        ...
        logger.info("gitlab_create_note: %s !%d", project_id, merge_request_iid)
    ...
    @mcp.tool()
    async def gitlab_mr_discussions(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        merge_request_iid: Annotated[int, Field(description="MR iid")],
        per_page: Annotated[int, Field(description="최대 결과 수", default=50)] = 50,
        ...
        logger.info("gitlab_mr_discussions: %s !%d", project_id, merge_request_iid)
    ...
    @mcp.tool()
    async def gitlab_create_merge_request_thread(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        merge_request_iid: Annotated[int, Field(description="MR iid")],
        ...
        logger.info("gitlab_create_merge_request_thread: %s !%d", project_id, merge_request_iid)
    ...
    async def gitlab_create_draft_note(
        ...
        position_json: Annotated[
            str,
            Field(description="코드 라인 position JSON (선택, gitlab_create_merge_request_thread와 동일)", default=""),
        ] = "",
        ...
        """MR 리뷰 초안(draft note)을 생성한다. gitlab_bulk_publish_draft_notes 로 일괄 게시할 수 있다."""
        logger.info("gitlab_create_draft_note: %s !%d", project_id, merge_request_iid)
        position = json.loads(position_json) if position_json else None
    ...
    @mcp.tool()
    async def gitlab_bulk_publish_draft_notes(
        ctx: Context,
        project_id: Annotated[str, Field(description="프로젝트 ID 또는 경로")],
        merge_request_iid: Annotated[int, Field(description="MR iid")],
    ) -> str:
        """현재 MR에 쌓인 내 draft note 들을 일괄 게시한다."""
        logger.info("gitlab_bulk_publish_draft_notes: %s !%d", project_id, merge_request_iid)
